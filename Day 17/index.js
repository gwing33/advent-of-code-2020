...
function getRelativeSides(cube, [x, y, z]) {
    const range = Array(3).fill().map((_, i) => i - 1);
        ...
        range.forEach(yOffset => {
            range.forEach(zOffset => {
                range.forEach(wOffset => {
                    ...
                    const _z = z + zOffset;
                    const _w = w + wOffset;
                    if (cube[_w] && cube[_w][_z] && cube[_w][_z][_y] && cube[_w][_z][_y][_x] === '#') {
...
function cycleCube(cube, times) {
    return Array(times).fill().reduce((_cube) => {
        const wrappedCube = addShell(_cube);
        return wrappedCube.reduce((wc, layer, z) => {
            layer.forEach((row, y) => {
                row.forEach((state, x) => {
                    const activeRelativeSides = getRelativeSides(wrappedCube, [x, y, z]);
                    if (state === '#') {
...
function getPart1Answer(cube) {
    ...
    return cycledCube.reduce((count, layer) => {
        return count + layer.reduce((c, row) => {
            return c + row.reduce((acc, state) => {
                if (state === '#') {
                    return acc + 1;
                }
                return acc;
...
function cycleHypercube(hypercube, times) {
    return Array(times).fill().reduce((_hypercube) => {
        const wrappedHypercube = addHypershell(_hypercube);
        return wrappedHypercube.reduce((whc, cube, w) => {
            cube.forEach((layer, z) => {
                layer.forEach((row, y) => {
                    row.forEach((state, x) => {
                        const activeRelativeSides = getRelativeHyperSides(wrappedHypercube, [x, y, z, w]);
                        if (state === '#') {
                            if (![2,3].includes(activeRelativeSides.length)) {
                                whc[w][z][y][x] = '.';
                            }
