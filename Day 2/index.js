function getLetterCount(arr, char) {
  return arr.reduce((acc, curr) => (curr === char ? acc + 1 : acc), 0);
}

function getPasswordParts(passwordLine) {
...
function getPart1Answer(passwordList) {
  return passwordList.reduce((acc, passwordLine) => {
    const [passwordArray, letter, min, max] = getPasswordParts(passwordLine);
    const counnt = getLetterCount(passwordArray, letter);
    return counnt >= min && counnt <= max ? acc + 1 : acc;
  }, 0);
}

function getPart2Answer(passwordList) {
  return passwordList.reduce((acc, passwordLine) => {
    const [passwordArray, letter, pos1, pos2] = getPasswordParts(passwordLine);
    if (
      passwordArray[pos1 - 1] !== passwordArray[pos2 - 1] &&
      (passwordArray[pos1 - 1] === letter || passwordArray[pos2 - 1] === letter)
    ) {
      return acc + 1;
    }
    return acc;
...
console.log("Part 1 Answer", getPart1Answer(passwordDatabase));
console.log("Part 2 Answer", getPart2Answer(passwordDatabase));
